...
const NewsPage: React.FC = () => {
  ...
  const newsCategories: Array<"Berita Teratas" | "Berita Terbaru"> = [
    "Berita Teratas",
    "Berita Terbaru",
  ];
  ...
  const sidebarNews =
    activeTab === "Berita Teratas" ? trendingNews : latestNews;

  const selectedTags = useMemo(
    () => buildNewsTags(selectedNews),
    [selectedNews],
  );
  ...
  const detailTrendingNews = useMemo(() => {
    return sortedNews
      ...
      .slice(0, 5);
  }, [sortedNews, selectedNews]);

  const detailRecommendedNews = useMemo(() => {
    if (!selectedNews) return [];

    const sameCategory = sortedNews.filter(
      (item) =>
        item.id !== selectedNews.id && item.category === selectedNews.category,
    );

    if (sameCategory.length >= 5) return sameCategory.slice(0, 5);

    const fallback = sortedNews.filter(
      (item) =>
        item.id !== selectedNews.id && item.category !== selectedNews.category,
    );
                ...
                <div className="p-6 md:p-8">
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    <span className="bg-[#0268ab] text-white text-xs font-semibold px-2.5 py-1 rounded-lg">
                      {CATEGORY_LABEL[selectedNews.category] ||
                ...
                <div className="lg:w-2/3">
                  {loading ? (
                    ...
                    <div
                      ...
                      onClick={() =>
                        featuredNews && handleNewsSelect(featuredNews)
                      }
                    >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-8">
                          <div className="flex items-center gap-3 mb-4">
                            <span className="bg-[#0268ab] text-white text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full">
                              {featuredNews
                                ? CATEGORY_LABEL[featuredNews.category] ||
                          ...
                          <h2 className="text-2xl lg:text-3xl font-bold text-white mb-4 leading-tight">
                            {featuredNews
                              ? formatTitlePrimary(featuredNews.title)
                              : "Belum ada berita"}
                          </h2>

                          <p className="text-white/90 text-sm mb-4 leading-relaxed">
                            {featuredNews
                              ? formatExcerptPrimary(featuredNews.excerpt)
                              : ""}
                          </p>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <button
                              onClick={() =>
                                featuredNews && setSelectedNews(featuredNews)
                              }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
                {loading
                  ? Array.from({ length: PAGE_SIZE }).map((_, idx) => (
                      <div
                        key={idx}
                        className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 animate-pulse space-y-3"
                      >
                        ...
                        <div className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 relative">
                          ...
                          <div className="p-4 relative z-10 flex flex-col h-40">
                            ...
                            <p className="text-xs text-gray-500 mb-3 line-clamp-2 grow">
                              {formatExcerptGrid(item.excerpt)}
                            </p>
