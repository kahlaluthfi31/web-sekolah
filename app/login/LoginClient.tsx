...
export default function LoginClient() {
  ...
  useEffect(() => {
    ...
    const map: Record<string, string> = {
      AccountInactive: 'Akun Anda belum aktif atau dinonaktifkan.',
      AccessDenied: 'Akses ditolak. Silakan gunakan akun yang terdaftar atau daftar terlebih dahulu.',
      OAuthAccountNotLinked: 'Email ini sudah terdaftar manual. Login dengan email & password.',
      Callback: 'Login gagal. Coba lagi.',
      ...
      GoogleNotRegistered: 'Email belum terdaftar. Silakan daftar akun terlebih dahulu.',
    }

    const friendly = map[errorParam] || 'Login gagal. Pastikan akun Anda terdaftar.'
    setFormStatus('error')
    setMessage(friendly)
  }, [searchParams])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await signIn('credentials', {
      ...
      email,
      password,
      // Tanda welcome untuk cred login
  ...
  const handleGoogleLogin = async () => {
    // Simpan intent di server agar terbaca saat callback
    await fetch('/api/auth/google-intent?mode=login', {
      method: 'POST',
      cache: 'no-store',
      credentials: 'include',
    }).catch(() => {})
    signIn('google', { callbackUrl: '/?welcome=back' })
  }

  // Sembunyikan form ketika sesi masih loading untuk menghindari flicker/hydration mismatch
