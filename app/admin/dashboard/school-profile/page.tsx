...
import {
  BookOpen, Target, Edit2, Trash2, Save, X,
  Loader2, Plus, History, FileText, Globe,
  MoreVertical, AlertTriangle, Upload, List, Type,
  Star, Award, Users, TrendingUp, Lightbulb, CheckCircle,
  Zap, Shield, Heart, Rocket, Trophy, Briefcase, GraduationCap, Building2,
  ImageIcon, Search, Wifi, Monitor, Cpu, FlaskConical, Music, Palette,
  Camera, Clock, Database, Code2, BookMarked, Handshake, Globe2, Leaf,
  Wrench, BarChart2, Medal, Landmark, Brain, Microscope, Plug, Bus,
  Bike, TreePine, Sun, Snowflake, Flame, Compass, Map, Flag,
} from 'lucide-react'
...
// ─── Keunggulan Icon Map ──────────────────────────────────
const KEUNGGULAN_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  // Umum
  Award, Star, Users, TrendingUp, Lightbulb, CheckCircle, Zap, Shield,
  Heart, Rocket, Trophy, Briefcase, GraduationCap, Building2, Target, BookOpen,
  // Teknologi & Sains
  Wifi, Monitor, Cpu, FlaskConical, Database, Code2, Microscope, Brain,
  // Pendidikan & Komunitas
  BookMarked, Handshake, Medal, Landmark, Globe2, Globe, Compass, Map, Flag,
  // Kreatif & Lainnya
  Music, Palette, Camera, Clock, Leaf, Wrench, BarChart2, Bus,
  Bike, TreePine, Sun, Snowflake, Flame, Plug,
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Kelola sejarah, visi misi, keunggulan, dan header halaman website.</p>
      </div>
...
function SejarahTab() {
  ...
  const [form, setForm] = useState({ year: '', title: '', description: '', sortOrder: 0 })
  ...
  const openCreate = () => {
    setEditingId(null)
    setForm({ year: '', title: '', description: '', sortOrder: nextSortOrder() })
    setError('')
    setShowModal(true)
  }
  ...
  useEffect(() => {
    ...
    return () => { _sejarahRef.openCreate = null }
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!j.success) { setError(j.message || 'Gagal menyimpan'); return }
      closeModal(); fetch_()
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true)
    try {
      await fetch(`/api/school-history/${deleteTarget.id}`, { method: 'DELETE' })
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <button onClick={openCreate} className="mt-3 text-[#0092DD] text-sm font-semibold hover:underline">+ Tambah sekarang</button>
          </div>
        ) : (
                ...
                {items.sort((a, b) => a.sortOrder - b.sortOrder).map(item => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-4 text-center">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-gray-100 text-gray-500 font-mono text-xs font-bold">{item.sortOrder}</span>
                    </td>
                    <td className="px-4 py-4">
                      <span className="inline-flex items-center justify-center px-3 py-1.5 rounded-full bg-[#0092DD] text-white text-xs font-bold whitespace-nowrap">{item.year}</span>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 text-center">
                      <div className="relative inline-block" data-sejarah-menu>
                        <button
                          data-sejarah-menu
                          onClick={(e) => {
                            e.stopPropagation()
                        ...
                        {openMenuId === item.id && (
                          ...
                          >
                            ...
                            >
                              <Edit2 className="w-3.5 h-3.5 text-[#0092DD]" /> Edit
                            </button>
                            <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-900">{editingId ? 'Edit Sejarah' : 'Tambah Sejarah'}</h2>
              <button onClick={closeModal} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Urutan Tampil <span className="text-red-500">*</span></label>
                ...
                <p className="text-[11px] text-gray-400 mt-1">Otomatis terisi, bisa diubah manual.</p>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={closeModal} className="flex-1 border border-gray-200 text-gray-600 px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-50 transition-colors">Batal</button>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
              ...
              <div className="text-center space-y-2">
                <h3 className="text-lg font-bold text-gray-900">Hapus Data Sejarah</h3>
                <p className="text-sm text-gray-500">Yakin ingin menghapus <span className="font-semibold text-gray-700">&lsquo;{deleteTarget.title}&rsquo;</span>?</p>
                <p className="text-xs text-gray-400">Tindakan ini tidak dapat dibatalkan.</p>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setDeleteTarget(null)} disabled={deleting}
                  className="flex-1 px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-all disabled:opacity-50">Batal</button>
...
function NoImagePlaceholder() {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-100 gap-1.5">
      ...
      <span className="text-[11px] text-gray-400 font-medium tracking-wide">No Image</span>
    </div>
  )
}
...
function CardImage({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    <Image
      src={src} alt={alt} fill unoptimized className="object-cover"
...
async function compressToWebP(file: File, quality = 0.75, maxDim = 1200): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob(blob => { if (blob) resolve(blob); else reject(new Error('Canvas toBlob failed')) }, 'image/webp', quality)
    }
    img.onerror = reject
    img.src = url
  })
...
function VisiMisiTab({ triggerCreate, onAllFilledChange }: { triggerCreate: number; onAllFilledChange: (v: boolean) => void }) {
  ...
  const [contentMode, setContentMode] = useState<'deskripsi' | 'poin'>('deskripsi')
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json()
      if (j.success) {
        const filtered = (j.data ?? []).filter((d: VisiMisiItem) => d.section === 'visi_misi')
        setItems(filtered)
  ...
  const resetForm = () => {
    setCardType('Visi'); setContentMode('deskripsi')
    setDescText(''); setPoints([''])
    setImageFile(null); setImagePreview(null); setExistingImageUrl(null)
    setImgError(false); setError('')
  }
  ...
  const openCreate = () => {
    ...
    if (existingTypes.includes('Visi') && !existingTypes.includes('Misi')) setCardType('Misi')
  ...
  const openEdit = (item: VisiMisiItem) => {
    ...
    if (item.content) {
      try {
        ...
        if (Array.isArray(parsed)) { mode = 'poin'; pts = parsed }
        else { desc = item.content }
      } catch { desc = item.content }
    }
    setContentMode(mode); setDescText(desc); setPoints(pts)
    setExistingImageUrl(item.image ?? null)
    setImagePreview(item.image ?? null)
    setImgError(false); setImageFile(null); setError('')
    setShowModal(true)
  }
  ...
  const uploadImage = async (): Promise<string | null> => {
    ...
    try {
      ...
      formData.append('file', new File([compressed], `visimisi-${Date.now()}.webp`, { type: 'image/webp' }))
      const res = await fetch('/api/upload', { method: 'POST', body: formData })
      const j = await res.json()
      if (j.success) return j.data.url
      setError('Gagal upload gambar: ' + (j.message ?? '')); return null
    } catch { setError('Gagal memproses gambar.'); return null }
    finally { setUploading(false) }
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const finalContent = contentMode === 'poin'
      ? JSON.stringify(points.filter(p => p.trim()))
      : descText.trim()
    if (!finalContent) { setError('Konten wajib diisi.'); return }
    // Gambar wajib
    if (!imagePreview && !existingImageUrl) { setError('Gambar wajib diunggah.'); return }
    ...
    try {
      ...
      const url = editingId ? `/api/school-profile/${editingId}` : '/api/school-profile'
      const res = await fetch(url, {
        method: editingId ? 'PUT' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ section: 'visi_misi', title: cardType, content: finalContent, image: imageUrl, orderPosition: existingTypes.includes('Visi') && existingTypes.includes('Misi') ? 0 : existingTypes.length }),
      })
      const j = await res.json()
      if (!j.success) { setError(j.message || 'Gagal menyimpan'); return }
      closeModal(); fetch_()
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true)
    try {
      await fetch(`/api/school-profile/${deleteTarget.id}`, { method: 'DELETE' })
  ...
  const parseContent = (content: string | null): { mode: 'deskripsi' | 'poin'; desc: string; pts: string[] } => {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading ? (
          ...
          items.sort((a, b) => a.orderPosition - b.orderPosition).map(item => {
            const { mode, desc, pts } = parseContent(item.content)
            const isMisi = item.title === 'Misi'
            return (
              <div key={item.id} className="bg-white rounded-2xl border border-gray-100 hover:shadow-md transition-shadow overflow-hidden">
                {/* Gambar */}
                <div className="relative h-44 bg-gray-100">
                  {/* Badge pojok kiri atas */}
                  <div className="absolute top-3 left-3 z-10">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm ${isMisi ? 'bg-[#F97316]' : 'bg-[#0092DD]'}`}>
                      {isMisi ? <List className="w-3 h-3" /> : <Target className="w-3 h-3" />}
                      {item.title}
                    </span>
                  </div>
                  ...
                  {/* 3-dot menu */}
                  <div className="absolute top-2 right-2 z-10" ref={openMenuId === item.id ? menuRef : null}>
                    <button
                    ...
                    {openMenuId === item.id && (
                      <div className="absolute right-0 top-9 bg-white rounded-xl shadow-lg border border-gray-100 py-1 w-36 z-20">
                        ...
                        <button onClick={() => { setOpenMenuId(null); setDeleteTarget(item) }}
                          className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">
                          <Trash2 className="w-3.5 h-3.5" /> Hapus
                        </button>
                      </div>
                ...
                <div className="p-4">
                  {mode === 'deskripsi' ? (
                    <p className="text-gray-600 text-sm leading-relaxed line-clamp-4">{desc}</p>
                  ) : (
                    <ul className="space-y-1.5">
                      {pts.slice(0, 4).map((pt, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs text-gray-600">
                          <span className={`mt-0.5 w-4 h-4 rounded-full shrink-0 flex items-center justify-center text-white text-[9px] font-bold ${isMisi ? 'bg-[#F97316]' : 'bg-[#0092DD]'}`}>{i + 1}</span>
                          <span className="line-clamp-2">{pt}</span>
                        </li>
                      ))}
                      {pts.length > 4 && <li className="text-xs text-gray-400 pl-6">+{pts.length - 4} poin lainnya…</li>}
                    </ul>
                  )}
                </div>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-900">{editingId ? 'Edit Item' : 'Tambah Item'}</h2>
              <button onClick={closeModal} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100"><X className="w-5 h-5" /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto px-6 py-5 space-y-5">
              ...
              {/* Tipe Card */}
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-2">Tipe Card <span className="text-red-500">*</span></label>
                <div className="grid grid-cols-2 gap-2">
                  {(['Visi', 'Misi'] as const).map(type => {
                    const alreadyExists = !editingId && existingTypes.includes(type)
                    const isSelected = cardType === type
                    return (
                      <button
                        key={type} type="button"
                        onClick={() => !alreadyExists && !editingId && setCardType(type)}
                      ...
                      >
                        {type === 'Misi' ? <List className="w-4 h-4" /> : <Target className="w-4 h-4" />}
                        {type}
                        {alreadyExists && <span className="text-[10px] ml-1">(sudah ada)</span>}
                      </button>
                    )
                  })}
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-2">Format Konten <span className="text-red-500">*</span></label>
                <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-1 w-fit">
                  <button type="button" onClick={() => setContentMode('deskripsi')}
                  ...
                  <button type="button" onClick={() => setContentMode('poin')}
              ...
              {contentMode === 'deskripsi' ? (
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Deskripsi <span className="text-red-500">*</span></label>
                  <textarea value={descText} onChange={e => setDescText(e.target.value)} rows={5}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-semibold text-gray-600">Poin-poin <span className="text-red-500">*</span> <span className="text-gray-400 font-normal">({points.length} item)</span></label>
                    <button type="button" onClick={() => setPoints(p => [...p, ''])}
                    ...
                    {points.map((pt, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <input type="text" value={pt}
                          onChange={e => setPoints(p => p.map((v, idx) => idx === i ? e.target.value : v))}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-2">
                  ...
                  <span className="text-gray-400 font-normal ml-1">(dikompresi ke WebP otomatis)</span>
                </label>
                ...
                {imagePreview && !imgError ? (
                  <div className="relative rounded-xl overflow-hidden bg-gray-100 h-36 group">
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                      ...
                      <button type="button" onClick={() => { setImageFile(null); setImagePreview(null); setExistingImageUrl(null); setImgError(false) }}
                    ...
                    {imageFile && (
                      <div className="absolute bottom-2 left-2 bg-black/60 text-white text-[10px] px-2 py-0.5 rounded-full">Akan dikompresi ke WebP</div>
                    )}
                  </div>
                ) : (
                  <button type="button" onClick={() => fileInputRef.current?.click()}
                    ...
                    <span className="text-[10px] text-gray-300">JPG, PNG, WEBP — dikonversi ke WebP</span>
                  </button>
                )}
              </div>
              ...
              <div className="flex gap-3 pt-1 pb-1">
                ...
                <button type="submit" disabled={saving || uploading}
                  ...
                  {uploading ? 'Mengompresi...' : saving ? 'Menyimpan...' : editingId ? 'Simpan Perubahan' : 'Tambah'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-gray-500">Yakin ingin menghapus item <span className="font-semibold text-gray-700">&lsquo;{deleteTarget.title}&rsquo;</span>?</p>
                <p className="text-xs text-gray-400">Tindakan ini tidak dapat dibatalkan.</p>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setDeleteTarget(null)} disabled={deleting}
...
function PageHeaderTab() {
  ...
  const [forms, setForms] = useState<Record<string, { title: string; subtitle: string }>>({})
  const [originals, setOriginals] = useState<Record<string, { title: string; subtitle: string }>>({})
  ...
  useEffect(() => {
    fetch('/api/page-headers')
      .then(r => r.json())
      .then(j => {
        const data: PageHeaderItem[] = j.success ? (j.data ?? []) : []
        const init: Record<string, { title: string; subtitle: string }> = {}
        ALL_PAGE_KEYS.forEach(key => {
          const existing = data.find(d => d.pageKey === key)
          init[key] = {
            title: existing?.title ?? PAGE_KEY_LABELS[key] ?? '',
  ...
  const handleSave = async (pageKey: string) => {
    ...
    try {
      ...
      const j = await res.json()
      if (j.success) {
        setOriginals(prev => ({ ...prev, [pageKey]: { ...f } }))
  ...
  const handleReset = (pageKey: string) => {
    const orig = originals[pageKey]
    if (orig) setForms(prev => ({ ...prev, [pageKey]: { ...orig } }))
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">Atur judul dan subjudul yang tampil di banner atas setiap halaman website.</p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {ALL_PAGE_KEYS.map(key => {
          const f = forms[key] ?? { title: '', subtitle: '' }
          const orig = originals[key] ?? { title: '', subtitle: '' }
          const isSaving = saving === key
          const isSaved = saved[key]
          const isDirty = f.title !== orig.title || f.subtitle !== orig.subtitle
          return (
            <div key={key} className="bg-white rounded-2xl px-6 py-5 border border-gray-100 hover:shadow-sm transition-shadow">
              {/* Header card */}
              <div className="flex items-center gap-2 mb-4">
                <FileText className="w-4 h-4 text-[#0092DD]" />
                <h3 className="text-sm font-bold text-gray-900">{PAGE_KEY_LABELS[key]}</h3>
                <span className="ml-auto text-[10px] font-mono text-gray-300 bg-gray-50 px-2 py-0.5 rounded-md">{key}</span>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={f.title}
                    onChange={e => setForms(prev => ({ ...prev, [key]: { ...f, title: e.target.value } }))}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0092DD]/30 focus:border-[#0092DD] transition"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5">
                    Subjudul <span className="text-gray-300 font-normal">(opsional)</span>
                  </label>
                  <textarea
                    value={f.subtitle}
                    onChange={e => setForms(prev => ({ ...prev, [key]: { ...f, subtitle: e.target.value } }))}
                    rows={3}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0092DD]/30 focus:border-[#0092DD] transition resize-none"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-1">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                  >
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all disabled:opacity-50 ${isSaved
...
function KeunggulanTab() {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json()
      if (j.success) {
        const filtered = (j.data ?? []).filter((d: KeunggulanItem) => d.section === 'keunggulan')
        setItems(filtered.sort((a: KeunggulanItem, b: KeunggulanItem) => a.orderPosition - b.orderPosition))
      }
    } finally { setLoading(false) }
  }, [])
  ...
  useEffect(() => {
    ...
    return () => { _keunggulanRef.openCreate = null }
  })
  ...
  const nextOrder = () => items.length === 0 ? 1 : Math.max(...items.map(i => i.orderPosition)) + 1
  ...
  async function handleSave() {
    if (!formTitle.trim()) { setError('Judul tidak boleh kosong'); return }
    if (!formDesc.trim()) { setError('Deskripsi tidak boleh kosong'); return }
    setSaving(true)
    setError('')
    try {
      const url = editingId ? `/api/school-profile/${editingId}` : '/api/school-profile'
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          image: formIcon,
          orderPosition: editingId ? items.find(i => i.id === editingId)?.orderPosition ?? nextOrder() : nextOrder(),
        }),
      })
      const j = await res.json()
      if (!j.success) { setError(j.message || 'Gagal menyimpan'); return }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">Kelola daftar keunggulan sekolah yang ditampilkan di halaman Profil Sekolah.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <div className="col-span-3 py-16 text-center bg-white rounded-2xl border border-gray-100">
            ...
            <p className="text-gray-400 text-sm">Belum ada keunggulan. Klik &ldquo;Tambah Keunggulan&rdquo; untuk mulai.</p>
          </div>
        ) : items.map(item => {
          return (
            <div key={item.id} className="bg-white rounded-2xl border border-gray-100 hover:shadow-md transition-shadow p-5 relative group">
              {/* 3-dot menu */}
              <div className="absolute top-3 right-3" ref={openMenuId === item.id ? menuRef : null}>
                <button
                  onClick={() => setOpenMenuId(openMenuId === item.id ? null : item.id)}
                ...
                {openMenuId === item.id && (
                  <div className="absolute right-0 top-9 bg-white rounded-xl shadow-lg border border-gray-100 py-1 w-36 z-20">
                    ...
                    <button onClick={() => { setDeleteTarget(item); setOpenMenuId(null) }}
              ...
              <div className="w-11 h-11 bg-[#77C5F0]/25 rounded-xl flex items-center justify-center mb-4">
                <IconComp name={item.image ?? 'Award'} className="w-5 h-5 text-[#0092DD]" />
              </div>

              {/* Urutan badge */}
              <span className="absolute top-3 left-4 text-[10px] font-bold text-gray-300">#{item.orderPosition}</span>

              <h3 className="text-sm font-bold text-gray-900 mb-1.5 pr-8">{item.title}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) closeModal() }}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10">
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              {error && (
                <div className="bg-red-50 border border-red-100 text-red-600 text-xs rounded-xl px-3 py-2.5">{error}</div>
              )}
              ...
              <div>
                ...
                <div className="relative mb-2">
                  ...
                  />
                  {iconSearch && (
                    <button type="button" onClick={() => setIconSearch('')}
                      ...
                      {filtered.map(key => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full aspect-square flex items-center justify-center rounded-xl border-2 transition-all ${selected
                              ? 'border-[#0092DD] bg-[#0092DD]/10 text-[#0092DD]'
                ...
                <div className="flex items-center gap-2 mt-2 px-3 py-2 bg-gray-50 rounded-xl">
                  {(() => { const Ic = KEUNGGULAN_ICONS[formIcon] ?? Award; return <Ic className="w-4 h-4 text-[#0092DD] shrink-0" /> })()}
                  <span className="text-[11px] text-gray-500">Terpilih: <span className="font-semibold text-gray-700">{formIcon}</span></span>
                </div>
              </div>
            </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm z-10 p-6 text-center">
            ...
            <h3 className="text-base font-bold text-gray-900 mb-1">Hapus Keunggulan?</h3>
            <p className="text-sm text-gray-500 mb-6">
              <span className="font-semibold text-gray-700">&ldquo;{deleteTarget.title}&rdquo;</span> akan dihapus permanen.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)} disabled={deleting}
