...
function ActionDropdown({ onDetail, onEdit, onDelete }: { onDetail?: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div ref={ref} className='relative inline-block'>
      <button ref={btnRef} onClick={handleToggle} className='p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-all'><MoreVertical className='w-4 h-4' /></button>
      {open && <div className={'absolute right-0 w-36 bg-white rounded-xl border border-gray-200 shadow-lg z-50 overflow-hidden ' + (dropUp ? 'bottom-full mb-1' : 'top-full mt-1')}>
        {onDetail && <button onClick={() => { setOpen(false); onDetail() }} className='flex items-center gap-2 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50'><Eye className='w-3.5 h-3.5' /> Detail</button>}
        <button onClick={() => { setOpen(false); onEdit() }} className='flex items-center gap-2 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50'><Edit2 className='w-3.5 h-3.5' /> Edit</button>
        <button onClick={() => { setOpen(false); onDelete() }} className='flex items-center gap-2 w-full px-4 py-2.5 text-sm text-red-600 hover:bg-red-50'><Trash2 className='w-3.5 h-3.5' /> Hapus</button>
      </div>}
    </div>
  )
}
