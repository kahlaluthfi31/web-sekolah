...
const QuickStats: React.FC = () => {
  ...
  return (
    <div className="relative -mt-16 z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center text-center hover:translate-y-[-5px] transition-transform duration-300">
            <div className="mb-4 p-4 bg-[#0092DD]/10 rounded-full">
              {stat.icon}
            </div>
      ...
      {/* Campus Event Banner */}
      <div className="mt-8 bg-[#0092DD] rounded-xl overflow-hidden shadow-lg flex flex-col md:flex-row items-stretch">
        ...
        <div className="flex-grow p-6 flex flex-col md:flex-row md:items-center justify-between text-white gap-4">
          ...
          <button className="whitespace-nowrap bg-white text-[#0092DD] px-6 py-2 rounded-full font-bold text-sm hover:bg-gray-100 transition-colors">
            Register
          </button>
        </div>
      </div>
